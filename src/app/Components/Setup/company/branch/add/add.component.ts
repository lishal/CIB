...
export class AddBranchComponent implements OnInit {
  ...
  private getUniqueProvinces(clusterid: string) {
    ...
    const selectedProvinceData = this.data.filter(
      (data) => data.idCluster === clusterid
    );
  ...
  private getUniqueDistrict(clusterid: string, provinceid: string) {
    ...
    const selectedDistrictData = this.data.filter(
      (data) => data.idCluster === clusterid && data.idProvince === provinceid
    );
